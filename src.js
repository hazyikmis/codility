...
//the only situation we should handle is coming 0 after 1
//2 cases:
//1-fish at the top of stack eats the coming fish - easy part
//  -nothing to do, continue withe the next fish
//2-coming fish eats the fish at the top of stack - hard part
//  -pop the eaten fish from stack
//  -compare the coming fish with the next top fish at the stack
//    -continue until:
//      -the coming fish eaten by a fish in the stack
//      -the coming fish pushed to the stack (maybe nothing left in the stack)

function fish(A, B) {
  if (A.length <= 1) return A.length; // if there is only 1 fish or no fish
  let stackA = []; //power
  let stackB = []; //direction
  ...
  for (i = 1; i < A.length; i++) {
    ...
    if (B[i] === 0 && stackB[stackB.length - 1] === 1) {
      //if (stackA[stackA.length-1] < A[i]) {
      while (stackA[stackA.length - 1] < A[i]) {
        stackA.pop();
        stackB.pop();
        if (stackB.length === 0 || stackB[stackB.length - 1] === 0) {
          stackA.push(A[i]);
          stackB.push(B[i]);
          break; //while loop
        }
      }
      //} //else do nothing, continue with the next coming fish
    } else {
      stackA.push(A[i]);
      stackB.push(B[i]);
    }
  ...
  //console.log(stackA, stackB);
  return stackA.length; // or return stackB.length;
}
